...
import static cc.ioctl.tmoe.util.Utils.logd;
import static cc.ioctl.tmoe.util.Utils.loge;
import static cc.ioctl.tmoe.util.Utils.logi;
                ...
                if (index != -1) {
                    ...
                    if (hostApp != null && component != null
                        ...
                        try {
                            ...
                            isTranslucent = ActProxyMgr.isWindowTranslucent(targetActivity);
                        } catch (ClassNotFoundException ignored) {
                        }
                        Intent wrapper = new Intent();
                        // we can't find other Activity class, so we only has one choice
                        wrapper.setClassName(component.getPackageName(), ActProxyMgr.STUB_TRANSLUCENT_ACTIVITY);
        ...
        public void callActivityOnCreate(Activity activity, Bundle icicle) {
            if (icicle != null) {
                ...
                if (ActProxyMgr.isModuleBundleClassLoaderRequired(className)) {
                    icicle.setClassLoader(Parasitics.class.getClassLoader());
                }
            }
