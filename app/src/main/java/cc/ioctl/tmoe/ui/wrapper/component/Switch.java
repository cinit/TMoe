...
public class Switch extends View {
    ...
    public Switch(Context context) {
        ...
        paint2.setStrokeWidth(LayoutHelper.dp(2));
    }
    ...
    public void setDrawRipple(boolean value) {
        ...
        if (rippleDrawable == null) {
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                ...
                maskDrawable = new Drawable() {
                    ...
                    public void draw(Canvas canvas) {
                        ...
                        canvas.drawCircle(bounds.centerX(), bounds.centerY(), LayoutHelper.dp(18), ripplePaint);
                    }
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                rippleDrawable.setRadius(LayoutHelper.dp(18));
            }
        ...
        if (Build.VERSION.SDK_INT >= 28 && value) {
            rippleDrawable.setHotspot(isChecked ? 0 : LayoutHelper.dp(100), LayoutHelper.dp(18));
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int width = LayoutHelper.dp(31);
        int thumb = LayoutHelper.dp(20);
        int x = (getMeasuredWidth() - width) / 2;
        float y = (getMeasuredHeight() - LayoutHelper.dpf2(14)) / 2;
        int tx = x + LayoutHelper.dp(7) + (int) (LayoutHelper.dp(17) * progress);
        ...
        for (int a = 0; a < 2; a++) {
            ...
            rectF.set(x, y, x + width, y + LayoutHelper.dpf2(14));
            canvasToDraw.drawRoundRect(rectF, LayoutHelper.dpf2(7), LayoutHelper.dpf2(7), paint);
            canvasToDraw.drawCircle(tx, ty, LayoutHelper.dpf2(10), paint);

            if (a == 0 && rippleDrawable != null) {
                rippleDrawable.setBounds(tx - LayoutHelper.dp(18), ty - LayoutHelper.dp(18), tx + LayoutHelper.dp(18), ty + LayoutHelper.dp(18));
        ...
        for (int a = 0; a < 2; a++) {
            ...
            canvasToDraw.drawCircle(tx, ty, LayoutHelper.dp(8), paint);
            ...
            if (a == 0) {
                if (iconDrawable != null) {
                    ...
                    tx -= LayoutHelper.dp(10.8f) - LayoutHelper.dp(1.3f) * progress;
                    ty -= LayoutHelper.dp(8.5f) - LayoutHelper.dp(0.5f) * progress;
                    int startX2 = (int) LayoutHelper.dpf2(4.6f) + tx;
                    int startY2 = (int) (LayoutHelper.dpf2(9.5f) + ty);
                    int endX2 = startX2 + LayoutHelper.dp(2);
                    int endY2 = startY2 + LayoutHelper.dp(2);

                    int startX = (int) LayoutHelper.dpf2(7.5f) + tx;
                    int startY = (int) LayoutHelper.dpf2(5.4f) + ty;
                    int endX = startX + LayoutHelper.dp(7);
                    int endY = startY + LayoutHelper.dp(7);
                    ...
                    startX = (int) LayoutHelper.dpf2(7.5f) + tx;
                    startY = (int) LayoutHelper.dpf2(12.5f) + ty;
                    endX = startX + LayoutHelper.dp(7);
                    endY = startY - LayoutHelper.dp(7);
                    canvasToDraw.drawLine(startX, startY, endX, endY, paint2);
                } else if (drawIconType == 2 || iconAnimator != null) {
                    paint2.setAlpha((int) (255 * (1.0f - iconProgress)));
                    canvasToDraw.drawLine(tx, ty, tx, ty - LayoutHelper.dp(5), paint2);
                    canvasToDraw.save();
                    canvasToDraw.rotate(-90 * iconProgress, tx, ty);
                    canvasToDraw.drawLine(tx, ty, tx + LayoutHelper.dp(4), ty, paint2);
