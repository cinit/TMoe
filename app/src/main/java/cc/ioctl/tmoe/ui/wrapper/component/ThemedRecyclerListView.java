 ...
 */
package cc.ioctl.tmoe.ui.wrapper.component;
...
import cc.ioctl.tmoe.ui.util.EmptyStubSpan;
import cc.ioctl.tmoe.util.Utils;

public class ThemedRecyclerListView extends RecyclerView {
    ...
    public abstract static class FastScrollAdapter extends SelectionAdapter {
        ...
        public abstract void getPositionForScrollProgress(ThemedRecyclerListView listView, float progress, int[] position);

        public void onStartFastScroll() {

        }

        public void onFinishFastScroll(ThemedRecyclerListView listView) {
        ...
        public float getScrollProgress(ThemedRecyclerListView listView) {
        ...
        public boolean fastScrollIsVisible(ThemedRecyclerListView listView) {
            return true;
        }
    ...
    public abstract static class SectionsAdapter extends FastScrollAdapter {
        ...
        /**
         * Please call this method before {@link Adapter#notifyDataSetChanged()}
         */
    ...
    public class FastScroll extends View {
        ...
        Runnable hideFloatingDateRunnable = new Runnable() {
            ...
            public void run() {
                if (pressed) {
                    Utils.cancelRunOnUIThread(hideFloatingDateRunnable);
                    Utils.runOnUIThread(hideFloatingDateRunnable, 4000);
                } else {
        ...
        public FastScroll(Context context, int type) {
            ...
            this.type = type;
            if (type == LETTER_TYPE) {
                letterPaint.setTextSize(LayoutHelper.dp(45));
                isRtl = LocaleController.isRTL();
            } else {
                isRtl = false;
                letterPaint.setTextSize(LayoutHelper.dp(13));
                letterPaint.setTypeface(LayoutHelper.getTypeface("fonts/rmedium.ttf"));
                paint2.setColor(Theme.getColor(Theme.key_windowBackgroundWhite));
                int resIdCalendarDate = context.getResources().getIdentifier("calendar_date", "drawable", context.getPackageName());
                if (resIdCalendarDate != 0) {
                    fastScrollBackgroundDrawable = ContextCompat.getDrawable(context, resIdCalendarDate).mutate();
                    ...
                    // is darwin really angry
                    throw new Resources.NotFoundException("R.drawable.calendar_date drawable not found");
                }
            }
            for (int a = 0; a < 8; a++) {
                radii[a] = LayoutHelper.dp(44);
            }

            scrollX = isRtl ? LayoutHelper.dp(10) : LayoutHelper.dp((type == LETTER_TYPE ? 132 : 240) - 15);
            updateColors();
            setFocusableInTouchMode(true);
            ViewConfiguration vc = ViewConfiguration.get(context);
            touchSlop = vc.getScaledTouchSlop();
            int resId = context.getResources().getIdentifier("fast_scroll_shadow", "drawable", context.getPackageName());
            if (resId != 0) {
                fastScrollShadowDrawable = ContextCompat.getDrawable(context, resId);
            } else {
                // is darwin really angry
                throw new Resources.NotFoundException("R.drawable.fast_scroll_shadow drawable not found");
            }
        }
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    float currentY = (float) Math.ceil((getMeasuredHeight() - LayoutHelper.dp(24 + 30)) * progress) + LayoutHelper.dp(12);
                    if (isRtl && x > LayoutHelper.dp(25) || !isRtl && x < LayoutHelper.dp(107) || lastY < currentY || lastY > currentY + LayoutHelper.dp(30)) {
                        return false;
                    }
                    if (type == DATE_TYPE && !floatingDateVisible) {
                        if (isRtl && x > LayoutHelper.dp(25) || !isRtl && x < (getMeasuredWidth() - LayoutHelper.dp(25)) || lastY < currentY || lastY > currentY + LayoutHelper.dp(30)) {
                            return false;
                        }
                    }
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (isMoving) {
                        ...
                        float minY = LayoutHelper.dp(12) + startDy;
                        float maxY = getMeasuredHeight() - LayoutHelper.dp(12 + 30) + startDy;
                        if (newY < minY) {
                            newY = minY;
                        } else if (newY > maxY) {
                            newY = maxY;
                        ...
                        float dy = newY - lastY;
                        lastY = newY;
                        progress += dy / (getMeasuredHeight() - LayoutHelper.dp(24 + 30));
                        if (progress < 0) {
                            progress = 0;
                        } else if (progress > 1) {
                            progress = 1;
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    invalidate();
                    if (adapter instanceof FastScrollAdapter) {
                        ((FastScrollAdapter) adapter).onFinishFastScroll(ThemedRecyclerListView.this);
                    }
                    showFloatingDate();
                    return true;
            }
            ...
            if (layoutManager instanceof LinearLayoutManager) {
                ...
                if (linearLayoutManager.getOrientation() == LinearLayoutManager.VERTICAL) {
                    ...
                    if (adapter instanceof FastScrollAdapter) {
                        ...
                        fastScrollAdapter.getPositionForScrollProgress(ThemedRecyclerListView.this, progress, positionWithOffset);
                        if (updatePosition) {
                        ...
                        if (newLetter == null) {
                            ...
                            if (letterLayout.getLineCount() > 0) {
                                ...
                                if (isRtl) {
                                    textX = LayoutHelper.dp(10) + (LayoutHelper.dp(88) - letterLayout.getLineWidth(0)) / 2 - letterLayout.getLineLeft(0);
                                } else {
                                    textX = (LayoutHelper.dp(88) - letterLayout.getLineWidth(0)) / 2 - letterLayout.getLineLeft(0);
                                }
                                textY = (LayoutHelper.dp(88) - letterLayout.getHeight()) / 2;
                            }
                        }
                    }
                }
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            setMeasuredDimension(LayoutHelper.dp(type == LETTER_TYPE ? 132 : 240), MeasureSpec.getSize(heightMeasureSpec));

            arrowPath.reset();
            arrowPath.setLastPoint(0, 0);
            arrowPath.lineTo(LayoutHelper.dp(4), -LayoutHelper.dp(4));
            arrowPath.lineTo(-LayoutHelper.dp(4), -LayoutHelper.dp(4));
            arrowPath.close();
        ...
        protected void onDraw(Canvas canvas) {
            int y = (int) Math.ceil((getMeasuredHeight() - LayoutHelper.dp(24 + 30)) * progress);
            rect.set(scrollX, LayoutHelper.dp(12) + y, scrollX + LayoutHelper.dp(5), LayoutHelper.dp(12 + 30) + y);
            if (type == LETTER_TYPE) {
                paint.setColor(ColorUtils.blendARGB(inactiveColor, activeColor, bubbleProgress));
                canvas.drawRoundRect(rect, LayoutHelper.dp(2), LayoutHelper.dp(2), paint);
            } else {
                paint.setColor(ColorUtils.blendARGB(Theme.getColor(Theme.key_windowBackgroundWhite), Color.WHITE, 0.1f));

                float cy = y + LayoutHelper.dp(12 + 15);
                ...
                canvas.drawCircle(scrollX + LayoutHelper.dp(8), y + LayoutHelper.dp(12 + 15), LayoutHelper.dp(24), paint);

                paint.setColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlackText));
                canvas.save();
                canvas.translate(scrollX + LayoutHelper.dp(4), y + LayoutHelper.dp(12 + 15 + 2 + 5) + LayoutHelper.dp(2) * bubbleProgress);
                canvas.drawPath(arrowPath, paint);
                canvas.restore();

                canvas.save();
                canvas.translate(scrollX + LayoutHelper.dp(4), y + LayoutHelper.dp(12 + 15 + 2 - 5) - LayoutHelper.dp(2) * bubbleProgress);
                canvas.rotate(180, 0, -LayoutHelper.dp(2));
                canvas.drawPath(arrowPath, paint);
                canvas.restore();
            }
            if (type == LETTER_TYPE) {
                if ((isMoving || bubbleProgress != 0)) {
                    ...
                    int progressY = y + LayoutHelper.dp(30);
                    y -= LayoutHelper.dp(46);
                    float diff = 0;
                    if (y <= LayoutHelper.dp(12)) {
                        diff = LayoutHelper.dp(12) - y;
                        y = LayoutHelper.dp(12);
                    }
                    float raduisTop;
                    float raduisBottom;
                    canvas.translate(LayoutHelper.dp(10), y);
                    if (diff <= LayoutHelper.dp(29)) {
                        raduisTop = LayoutHelper.dp(44);
                        raduisBottom = LayoutHelper.dp(4) + (diff / LayoutHelper.dp(29)) * LayoutHelper.dp(40);
                    } else {
                        diff -= LayoutHelper.dp(29);
                        raduisBottom = LayoutHelper.dp(44);
                        raduisTop = LayoutHelper.dp(4) + (1.0f - diff / LayoutHelper.dp(29)) * LayoutHelper.dp(40);
                    }
                    if (isRtl && (radii[0] != raduisTop || radii[6] != raduisBottom) || !isRtl && (radii[2] != raduisTop || radii[4] != raduisBottom)) {
                        ...
                        path.reset();
                        rect.set(isRtl ? LayoutHelper.dp(10) : 0, 0, LayoutHelper.dp(isRtl ? 98 : 88), LayoutHelper.dp(88));
                ...
                if (letterLayout != null && floatingDateProgress != 0) {
                    ...
                    canvas.scale(s, s, rect.right - LayoutHelper.dp(12), rect.centerY());

                    float cy = rect.centerY();
                    float x = rect.left - LayoutHelper.dp(30) * bubbleProgress - LayoutHelper.dp(8);
                    float r = letterLayout.getHeight() / 2f + LayoutHelper.dp(6);
                    float width = replaceLayoutProgress * letterLayout.getWidth() + fromWidth * (1f - replaceLayoutProgress);
                    rect.set(x - width - LayoutHelper.dp(36), cy - letterLayout.getHeight() / 2f - LayoutHelper.dp(8), x - LayoutHelper.dp(12), cy + letterLayout.getHeight() / 2f + LayoutHelper.dp(8));
                    ...
                    if (replaceLayoutProgress != 1f) {
                        canvas.save();
                        rect.inset(LayoutHelper.dp(4), LayoutHelper.dp(2));
                        canvas.clipRect(rect);
                        if (outLetterLayout != null) {
                            letterPaint.setAlpha((int) (oldAlpha2 * floatingDateProgress * (1f - replaceLayoutProgress)));
                            canvas.save();
                            canvas.translate(x - outLetterLayout.getWidth() - LayoutHelper.dp(24), cy - outLetterLayout.getHeight() / 2f + (fromTop ? -1 : 1) * LayoutHelper.dp(15) * replaceLayoutProgress);
                        ...
                        if (inLetterLayout != null) {
                            ...
                            canvas.save();
                            canvas.translate(x - inLetterLayout.getWidth() - LayoutHelper.dp(24), cy - inLetterLayout.getHeight() / 2f + (fromTop ? 1 : -1) * LayoutHelper.dp(15) * (1f - replaceLayoutProgress));
                        ...
                        if (stableLetterLayout != null) {
                            ...
                            canvas.save();
                            canvas.translate(x - stableLetterLayout.getWidth() - LayoutHelper.dp(24), cy - stableLetterLayout.getHeight() / 2f);
                        ...
                        letterPaint.setAlpha((int) (oldAlpha2 * floatingDateProgress));
                        canvas.save();
                        canvas.translate(x - letterLayout.getWidth() - LayoutHelper.dp(24), cy - letterLayout.getHeight() / 2f + LayoutHelper.dp(15) * (1f - replaceLayoutProgress));
        ...
        public void showFloatingDate() {
            ...
            Utils.cancelRunOnUIThread(hideFloatingDateRunnable);
            Utils.runOnUIThread(hideFloatingDateRunnable, 2000);
        }
        ...
        public int getScrollBarY() {
            return (int) Math.ceil((getMeasuredHeight() - LayoutHelper.dp(24 + 30)) * progress) + LayoutHelper.dp(17);
        }
    ...
    private class ThemedRecyclerListViewItemClickListener implements OnItemTouchListener {

        public ThemedRecyclerListViewItemClickListener(Context context) {
            gestureDetector = new GestureDetectorFixDoubleTap(context, new GestureDetector.SimpleOnGestureListener() {
                ...
                private void onPressItem(View cv, MotionEvent e) {
                    if (cv != null && (onItemClickListener != null || onItemClickListenerExtended != null)) {
                        ...
                        Utils.runOnUIThread(clickRunnable = new Runnable() {
                            @Override
                            public void run() {
                        ...
                        if (selectChildRunnable != null) {
                            Utils.cancelRunOnUIThread(selectChildRunnable);
        ...
        public boolean onInterceptTouchEvent(RecyclerView view, MotionEvent event) {
            ...
            boolean isScrollIdle = ThemedRecyclerListView.this.getScrollState() == ThemedRecyclerListView.SCROLL_STATE_IDLE;
            ...
            if (action == MotionEvent.ACTION_DOWN || action == MotionEvent.ACTION_POINTER_DOWN) {
                if (!interceptedByChild && currentChildView != null) {
                    ...
                    Utils.runOnUIThread(selectChildRunnable, ViewConfiguration.getTapTimeout());
    ...
    public void cancelClickRunnables(boolean uncheck) {
        ...
        if (clickRunnable != null) {
            Utils.cancelRunOnUIThread(clickRunnable);
            clickRunnable = null;
        }
        interceptedByChild = false;
    }
    ...
    public ThemedRecyclerListView(Context context) {
        super(context);

        // setGlowColor(getThemedColor(Theme.key_actionBarDefault)) what is this f***?
        ...
        addOnItemTouchListener(new ThemedRecyclerListViewItemClickListener(context));
    }
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        if (fastScroll != null) {
            ...
            fastScroll.measure(MeasureSpec.makeMeasureSpec(LayoutHelper.dp(132), MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY));
        }
                    ...
                    if (sectionsAdapter != null) {
                        ...
                        if (sectionsType == 1) {
                            ...
                            for (int a = 0; a < childCount; a++) {
                                ...
                                if (bottom < sectionOffset + paddingTop + LayoutHelper.dp(32)) {
                                    continue;
                                }
                            ...
                            for (int a = startSection; a < startSection + sectionsCount; a++) {
                                ...
                                if (a == startSection) {
                                    ...
                                    if (child != null) {
                                        ...
                                        header.setTag(-LayoutHelper.dp(100));
                                    }
                                    itemNum += count;
                                }
                            }
                        ...
                        if ((scrollingByUser || force) && fastScroll != null && !fastScroll.isPressed()) {
                            ...
                            if (adapter instanceof FastScrollAdapter) {
                                float p = ((FastScrollAdapter) adapter).getScrollProgress(ThemedRecyclerListView.this);
                                boolean visible = ((FastScrollAdapter) adapter).fastScrollIsVisible(ThemedRecyclerListView.this);
    ...
    public void highlightRow(ThemedRecyclerListView.IntReturnCallback callback) {
        highlightRowInternal(callback, true);
    }

    private void highlightRowInternal(ThemedRecyclerListView.IntReturnCallback callback, boolean canHighlightLater) {
        if (removeHighlighSelectionRunnable != null) {
            Utils.cancelRunOnUIThread(removeHighlighSelectionRunnable);
        ...
        if (holder != null) {
            ...
            Utils.runOnUIThread(removeHighlighSelectionRunnable = () -> {
                removeHighlighSelectionRunnable = null;
        ...
        if (overlayContainer == null) {
            overlayContainer = new FrameLayout(getContext()) {
                ...
                public void requestLayout() {
                    super.requestLayout();
                    try {
                        final int measuredWidth = ThemedRecyclerListView.this.getMeasuredWidth();
                        final int measuredHeight = ThemedRecyclerListView.this.getMeasuredHeight();
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (multiSelectionGesture && e.getAction() != MotionEvent.ACTION_DOWN && e.getAction() != MotionEvent.ACTION_UP && e.getAction() != MotionEvent.ACTION_CANCEL) {
            ...
            if (multiSelectionGestureStarted) {
                ...
                if (e.getY() > getMeasuredHeight() - LayoutHelper.dp(56) - listPaddings[1] && !(currentSelectedPosition < startSelectionFrom && multiSelectionListener.limitReached())) {
                    startMultiselectScroll(false);
                } else if (e.getY() < LayoutHelper.dp(56) + listPaddings[0] && !(currentSelectedPosition > startSelectionFrom && multiSelectionListener.limitReached())) {
    ...
    private boolean chekMultiselect(float x, float y) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (useRelativePositions) {
                ...
                LayoutHelper.rectTmp.set(child.getLeft(), child.getTop(), child.getLeft() + child.getMeasuredWidth(), child.getTop() + child.getMeasuredHeight());

                if (LayoutHelper.rectTmp.contains(x, y)) {
    ...
    private void cancelMultiselectScroll() {
        ...
        Utils.cancelRunOnUIThread(scroller);
    }
